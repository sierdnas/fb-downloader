...
![Download tab — desktop](screenshots/download-desktop.png)

| Analysis results | Download queue |
|---|---|
| ![Analysis results with descriptions and tags](screenshots/download-results.png) | ![Download queue in progress](screenshots/download-queue.png) |

| History | Settings |
|---|---|
| ![Previously analyzed links and download history](screenshots/history.png) | ![Settings tab](screenshots/settings.png) |

| Log | Login |
|---|---|
| ![In-app log viewer](screenshots/log.png) | ![Login / session status](screenshots/login.png) |
...
- **Jellyfin metadata**: videos/reels are organized as episodes of a TV
  ...
  - `poster.jpg` copied into each `Stagione NNNN/` season folder too
    (same image as the show-level poster — there's no per-season
    Facebook asset to use instead, seasons are just publish years), so
    Jellyfin shows branded artwork per season instead of a generic
    default. Skipped if the show poster itself couldn't be fetched;
    never overwrites a season poster that's already there.
  ...
  - `<episode-filename>-thumb.jpg` next to each downloaded video/reel: a
    frame extracted with FFmpeg at the ~10th second (Jellyfin/Kodi's
    naming convention for a local episode thumbnail — used directly
    instead of Jellyfin generating its own, which fixes short clips
    showing up with no thumbnail at all in the library grid). For clips
    shorter than 10 seconds, grabs a frame partway through instead of
    failing. Never overwrites an existing thumbnail.
...
## Using it on Android (installable as an app)

The web UI is a PWA (Progressive Web App): with a valid manifest, icons,
and a minimal service worker, Android/Chrome can install it as a
standalone app instead of just a browser bookmark — icon on the home
screen, opens without the browser's address bar.

**To install:** open the app in Chrome on Android, tap the three-dot
menu → "Add to Home screen" / "Install app".

**Important caveat:** service workers (needed for full installability)
require a *secure context* — HTTPS, or `localhost`. If you access the
app over Tailscale with a plain IP (`http://100.x.x.x:9797`), Chrome may
not offer the full standalone install, since that's treated as an
insecure origin. Two easy ways to fix this, since a self-hosted setup
usually already has one of these available:

- **Tailscale Serve**: if you already use Tailscale, `tailscale serve
  https / http://localhost:9797` (run on the NAS) exposes the app over
  HTTPS at `https://<device>.<your-tailnet>.ts.net` with a certificate
  Tailscale manages automatically — no extra setup needed.
- **A reverse proxy with automatic HTTPS** (e.g. Caddy), if you already
  run one for other services on your network — point it at
  `http://<nas-ip>:9797` and let it handle the TLS certificate.

Without either of these, the app still works fine on Android through the
regular browser (the responsive layout with the mobile hamburger menu
applies regardless) — you just won't get the full "installed app" feel.

## Description translation (optional)

The Settings tab has a **"Translate description into interface
language"** checkbox (off by default). When enabled, each post's
description is translated into the current UI language at analysis
time — the translated text is what shows up in the results table, and
(since it flows through the same data all the way to the download
step) it's also what ends up in the `.nfo`'s `<plot>` field for
Jellyfin, with no separate translation step needed at download time.

**Honest limitations:**
- Uses Google Translate's free, unofficial web endpoint (via the
  `deep-translator` library) — no API key needed, but also no SLA:
  Google can rate-limit, block, or change it at any time without
  notice. When a translation fails for any reason, the app silently
  falls back to the original, untranslated text — never an error, never
  a blocked analysis.
- Requires the container to have outbound internet access to Google's
  servers (a Tailscale-only NAS without a general internet route won't
  be able to reach it).
- Adds one network round-trip per item being analyzed, so analyzing a
  profile/page with many posts gets noticeably slower with this
  enabled.
- Only affects **new** analyses going forward — re-download an
  already-downloaded item to regenerate its `.nfo` with a translation
  if you enable this after the fact.
